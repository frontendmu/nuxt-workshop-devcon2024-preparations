...
async function diagnostics(workspace, logLevel, options = { watch: false, verbose: false, config: null }) {
  ...
  let workspaceUri;
  if (workspace) {
    const absPath = import_node_path.default.resolve(process.cwd(), workspace);
...
class TestStream extends import_node_stream.Duplex {
...
async function getDiagnostics(workspaceUri, severity, options) {
  ...
  const absFilePaths = files.map((f) => import_node_path.default.resolve(workspaceUri.fsPath, f));
  ...
  await Promise.all(
    absFilePaths.map(async (absFilePath) => {
      const fileText = await import_node_fs.default.promises.readFile(absFilePath, "utf-8");
      ...
      try {
        ...
        if (diagnostics2.length > 0) {
          logChunk += import_node_os.default.EOL + diagnostics2.map(
  ...
  watcher.on("all", async (event, filePath) => {
    const extname = import_node_path.default.extname(filePath);
