  ...
  return {
    ...
    async configureServer({ root }) {
      ...
      if (!pluginConfig.stylelint)
        return;
      const translatedOptions = await (0, import_options.translateOptions)(pluginConfig.stylelint.lintCommand);
      const baseConfig = {
        cwd: root,
        ...translatedOptions
      };
      ...
      const handleFileChange = async (filePath, type) => {
        ...
        if (type === "unlink") {
          ...
          const { results: diagnosticsOfChangedFile } = await import_stylelint.default.lint({
            ...baseConfig,
            files: filePath
          });
          const newDiagnostics = diagnosticsOfChangedFile.flatMap(
            (d) => (0, import_logger.normalizeStylelintDiagnostic)(d)
          );
      ...
      manager.initWith(diagnostics.flatMap((p) => (0, import_logger.normalizeStylelintDiagnostic)(p)));
      dispatchDiagnostics();
